...
app.get('/api/admin/shopee/auth-url', async (c) => {
  try {
    ...
    const authUrl = `https://openplatform.sandbox.test-stable.shopee.sg${path}?partner_id=${partnerId}&timestamp=${timestamp}&sign=${sign}&redirect=${encodeURIComponent(redirectUrl)}`
...
app.get('/api/shopee/callback', async (c) => {
  ...
  try {
    ...
    if (c.env.SHOPEE_PARTNER_ID !== 'YOUR_PARTNER_ID') {
      ...
      const tokenRes = await fetch(`https://openplatform.sandbox.test-stable.shopee.sg${path}?partner_id=${partnerId}&timestamp=${timestamp}&sign=${sign}`, {
        method: 'POST',
